...
class AppModule(appModuleHandler.AppModule):
	...
	def searchList(self, attr, lo):
		fg= api.getForegroundObject()
		obj= next((child for child in fg.getChild(1).recursiveDescendants if hasattr(child, 'UIAAutomationId') and child.UIAAutomationId == attr), None)
		if obj:
			if lo: self.list_object= obj
			return obj
	...
	def script_messageHistory(self, gesture):
		...
		if not self.list_object: self.searchList('Messages', True)
		try:
	...
	def script_chatFocus(self, gesture):
		...
		if self.chat_object != None:
			...
			chat_object= self.searchList('ChatsList', False)
			if chat_object:
				chat_object.getChild(1).setFocus()
				message(chat_object.getChild(1).name)
				speak(0.1)
	...
	def script_messagesFocus(self, gesture):
		if SOUNDS: playWaveFile(os.path.join(SOUNDS_PATH, 'click.wav'))
		self.list_object= self.searchList('Messages', True)
	...
	def script_unreadFocus(self, gesture):
		if not self.list_object:
			list= self.searchList('Messages', True)
		else:
			list= self.list_object
		if SOUNDS: playWaveFile(os.path.join(SOUNDS_PATH, 'click.wav'))
		unreadObj= next((obj for obj in reversed(list.children) if hasattr(obj.firstChild, 'role') and obj.firstChild.role == controlTypes.Role.BUTTON), None)
		if unreadObj:
			unreadObj.setFocus()
		else:
	...
	def script_profile(self, gesture):
		if not self.list_object: self.searchList('Messages', True)
		obj= next((obj for obj in self.list_object.parent.children if hasattr(obj, 'UIAAutomationId') and obj.UIAAutomationId == 'Profile'), None)
		if obj:
			obj.setFocus()
			obj.doAction()
	...
	def script_pinnedChats(self, gesture):
		if self.list_object == None: self.searchList('Messages', True)
